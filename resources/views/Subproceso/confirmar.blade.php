...
@endphp
<div class="container">
  <h3>¿Desea eliminar el subproceso {{$subproceso->sub_nombre}} ?</h3>
  <form method="POST" action="{{ route('subproceso.destroy', $subproceso->sub_id)}}">
    @method ('delete')
    @csrf
    <button type="submit" class="btn btn-danger"><i class="fa fa-check-square"></i> SI</button>
    <a href="{{route('cancelasubproceso', $subproceso->emp_ruc)}}" class="btn btn-primary" ><i class="fa fa-times-circle"></button></i> NO</a>
  </form>
</div>
@endsection
