...
@endphp
<div class="container">
  <h3>¿Desea eliminar la estrategia {{$estrategia->est_nombre}} ?</h3>
  <form method="POST" action="{{ route('estrategia.destroy', $estrategia->est_id)}}">
    @method ('delete')
    @csrf
    <button type="submit" class="btn btn-danger"><i class="fa fa-check-square"></i> SI</button>
    <a href="{{route('cancelaestrategia', $estrategia->sub_id)}}" class="btn btn-primary" ><i class="fa fa-times-circle"></button></i> NO</a>
  </form>
</div>
@endsection
