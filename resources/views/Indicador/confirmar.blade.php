...
@endphp
<div class="container">
  <h3>¿Desea eliminar el indicador {{$indicador->ind_nombre}} ?</h3>
  <form method="POST" action="{{ route('indicador.destroy', $indicador->ind_id)}}">
    @method ('delete')
    @csrf
    <button type="submit" class="btn btn-danger"><i class="fa fa-check-square"></i> SI</button>
    <a href="{{route('cancelaindicador', $indicador->emp_ruc)}}" class="btn btn-primary" ><i class="fa fa-times-circle"></button></i> NO</a>
  </form>
</div>
@endsection
